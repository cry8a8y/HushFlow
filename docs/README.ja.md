...
AIツールに任意のプロンプトを送信してください。初回はガイド付きセットアップウィザードが呼吸エクササイズとテーマの選択を案内します。セットアップ後、AI思考時に呼吸アニメーションが自動的に表示されます。
...
# ガイド・診断
hushflow onboarding            # 初回セットアップウィザードを再実行
hushflow doctor                # インストール状態と環境をチェック
hushflow --version             # バージョンを表示
```
